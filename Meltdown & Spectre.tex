...
\texttt{\_mm\_clflush} ~\cite{intelintrinsics} to clean the probe array from the cache. This part resets the cache state
...
If the measured access time is smaller or equal to the threshold the related index of the element is considered as a potential result.
...
Tricking the branch predictor is the core of the Spectre attack. Listing ~\ref{lst:twiddling} 
...
attacked program which is multiplied by the secret value of \texttt{array1}. So an element of \texttt{array2}, the probe array, is loaded
and thus cached if the instruction is executed.
