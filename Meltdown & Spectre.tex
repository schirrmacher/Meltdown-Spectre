...
\subsection{Observing Microarchitectural CPU State}
...
Despite modern processors do not eliminate all microarchitectural state changes properly so that the execution of a transient instruction
 ...
 The resulting microarchitectural state change is not vanished by modern CPUs which leads to the birth of \textit{cache attacks} ~\cite{osvik2006cache}.
...
can be extracted from the microarchitectural state as highlighted in the next section. 
...
Meltdown and Spectre exploit the execution of transient instructions by accessing invalid memory locations 
of other processes which are usually protected by the CPU and the operating system.
...
and vanishes the invalid accessed data from its current state when the illegal action is detected.
But the microarchitectural state of the cache is unchanged and thus leaves a trace of the secret byte. 
Meltdown and Spectre instrumenting techniques like Flush+Reload ~\cite{yarom2014flush} 
or Evict+Reload ~\cite{gruss2015cache} to reveal the secret by analyzing the timing behavior of the memory access.
If a memory location is cached it is loaded significantly faster into a register of the CPU than data loaded
from memory. 

Both attacks work like explained in the following:
We have to create an array with as many elements
as the size of a byte and load a single element from the array depending on the secret by adding it to 
the starting position of the array. Thus the secret represents the offset of the target element 
which will be accessed. If we can identify the accessed element we know the secret value because it equals
the index of the accessed element. Since memory is cached when being read and the processor does not clean 
the cache state after an illegal memory access we just have to search the cached element 
for extracting the secret value. In chapter 4 this process is examined in detail with the help of the 
implementation of both attacks.
