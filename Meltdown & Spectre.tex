...
is the secret byte.
Then techniques like Flush+Reload can be applied to reveal the secret byte from kernel memory.
Line 5 shifts the content of the \texttt{RAX} register by the size of a memory page to access the next element of the probe array.
This is necessary to ensure that the distance of the accessed elements is large enough and avoids caching of adjacent memory locations.
...
\begin{lstlisting}[language=C, caption=Transient Instruction Sequence of Meltdown, label={lst:reload}]
